...
namespace structures 
{
	...
	Array<T>::~Array()
	{
		delete vector_; // vymazem vektor
		vector_ = nullptr; // smernik ktory ukazuje nikde
		size_ = 0;
	}
	...
	{
		return new Array<T>(*this); //this je smernik tak ho dereferencujeme
	}
	...
	{
		if (this != &other) { // tu zistujem ze ci nie su rovnake adresy, this je adresa mojho &other adresa druheho
			if (size_ != other.size_) { //zistujem ci polia maju rovnake velkosti 
				throw std::invalid_argument("sizes of arrays are different"); // vynimka tu je preto kebyze velkosti poli niesu rovnake
			}

			*vector_ = *other.vector_; // obsah jedneho vektoru dávam do druheho vektoru 
		}
		return *this;
	}
